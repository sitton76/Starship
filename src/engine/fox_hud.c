...
void HUD_MatrixTranslateCoordLeft(f32* transX, f32* transY) {
    *transX = OTRGetRectDimensionFromLeftEdgeCentered(*transX) - (SCREEN_WIDTH / 2.0f);
...
void HUD_LivesCount2_Draw(f32 x, f32 y, s32 number) {
    ...
    if (gShowLevelClearStatusScreen == 0) {
        x0 = OTRGetDimensionFromRightEdgeCentered(x0);
        x1 = OTRGetDimensionFromRightEdgeCentered(x1);
        x2 = OTRGetDimensionFromRightEdgeCentered(x2);
    }
...
void HUD_LevelClearStatusScreen_Draw(void) {
    ...
    if (gShowLevelClearStatusScreen == true) {
        ...
        HUD_Number_Draw(OTRGetRectDimensionFromLeftEdgeCentered(24.0f), 30.0f + 3.0f, D_801617C0[5], 1.0f, false, 999);
...
void HUD_PauseScreen_Update(void) {
    ...
    if ((gPlayState == PLAY_PAUSE) && (gLevelStartStatusScreenTimer == 0) && !gVersusMode) {
        switch (sPauseScreenIwork[0]) {
            ...
            case 3:
                Graphics_FillRectangle(&gMasterDisp, OTRGetRectDimensionFromLeftEdgeCentered(0), 0,
                                       OTRGetDimensionFromRightEdgeCentered(SCREEN_WIDTH), SCREEN_HEIGHT, 0, 0, 0, 255);
...
s32 HUD_RadarMarks_Update(void) {
    ...
    switch (gCurrentLevel) {
        case LEVEL_CORNERIA:
            gHudOffsetRect = OTRGetDimensionFromRightEdgeCentered(0.0f);
...
void HUD_RadioCharacterName_Draw(void) {
    if (gGameState == GSTATE_PLAY) {
        ...
        f32 xPos = OTRGetRectDimensionFromLeftEdgeCentered(73.0f);
...
void HUD_PlayerShieldGauge_Draw(f32 x, f32 y) {
    x = OTRGetRectDimensionFromLeftEdgeCentered(x);
    RCP_SetupDL(&gMasterDisp, SETUPDL_75);
    gDPSetPrimColor(gMasterDisp++, 0, 0, 255, 255, 255, 255);
...
void HUD_BoostGauge_Draw(f32 xPos, f32 yPos) {
    ...
    if (gVersusMode) {
        ...
        switch (gPlayerNum) {
            case 1:
                xPos = OTRGetRectDimensionFromLeftEdgeCentered(xPos);
                break;
            case 2:
                xPos = OTRGetRectDimensionFromLeftEdgeCentered(xPos);
                break;
            case 3:
                xPos = OTRGetDimensionFromRightEdgeCentered(xPos);
                break;
            case 4:
                xPos = OTRGetDimensionFromRightEdgeCentered(xPos);
                break;
...
void HUD_DrawBossHealth(void) {
    ...
    if ((gShowBossHealth == 1) && (gTeamShields[TEAM_ID_SLIPPY] > 0)) {
        ...
        temp2 = OTRGetRectDimensionFromLeftEdgeCentered(temp2);
        temp4 = OTRGetRectDimensionFromLeftEdgeCentered(temp4);

        RCP_SetupDL(&gMasterDisp, SETUPDL_78_POINT);
        gDPSetPrimColor(gMasterDisp++, 0, 0, 255, 255, 255, 255);
        Lib_TextureRect_CI4(&gMasterDisp, D_1011A40, D_1011AB0, 32, 7, temp2, temp3, 1.0f, 1.0f);
        f32 border = sp3C;
        border = OTRGetRectDimensionFromLeftEdgeCentered(border);
        RCP_SetupDL(&gMasterDisp, SETUPDL_76_POINT);
        ...
        if (D_801616C4 >= 0.88f) {
            ...
            temp6 = OTRGetRectDimensionFromLeftEdgeCentered(temp6);
            if (D_801616C8 > 0.0f) {
                // LTODO: FIX BOSS BAR
...
void HUD_BombCounter_Draw(f32 x, f32 y) {
    ...
    x = OTRGetDimensionFromRightEdgeCentered(x);
